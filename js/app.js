...
const TOOLTIPS = {
    ...
    'margin-by-type': {
        ...
        description: 'Показывает процент прибыли от выручки отдельно для новых товаров и секонд-хенда. Тип определяется галочкой б/у в карточке, а не поставщиком. Это ключевая метрика – показывает, где больше прибыли на вложенный рубль.',
        example: 'Пример: Новые товары дают маржу 25%, а секонд – 55%. Значит, на каждый вложенный рубль в секонд вы получаете больше прибыли, несмотря на меньшую выручку.'
    },
    'markup-coefficient': {
    ...
    'abc-analysis': {
        title: 'ABC-анализ',
        description: 'Классификация товаров по принципу Парето. <strong>Группа A</strong> – 20% товаров, дающих 80% выручки/маржи (звёзды). <strong>Группа B</strong> – следующие 15% (середнячки). <strong>Группа C</strong> – оставшиеся 5% (аутсайдеры). Можно фильтровать по состоянию (новые/секонд).',
    ...
    'brand-turnover': {
        ...
        description: 'Показывает, за сколько дней в среднем продаётся весь запас каждого бренда. Помогает понять, какие бренды "крутятся" быстро, а какие замораживают деньги. Можно отдельно посмотреть новые и б/у товары одного бренда.',
    ...
    'sold-percentage': {
        ...
        description: 'Из всех поступивших вещей за последние 6 месяцев – сколько процентов продалось. Показывает эффективность закупок отдельно для новых товаров и секонда по состоянию карточки.',
        example: 'Пример: Из 100 поступивших вещей за 6 месяцев продалось 75 – это 75%. Норма для новых товаров: 60-70%, для секонда: 70-80%.'
    },
    'stale-products': {
        title: 'Залежавшиеся товары',
        description: 'Товары с остатком, но без продаж сверх порога. Для новых товаров порог – 90 дней, для секонд-хенда – 120 дней (т.к. уникальные вещи ищут своего клиента дольше). Показывает замороженные деньги.',
    ...
    'permanent-supplier': {
        ...
        description: 'Отметьте, если товары этого бренда можно регулярно дозаказывать. Признак поставщика подставляется при выборе бренда. Состояние конкретной вещи задаётся отдельно галочкой «Этот товар – б/у (секонд)».',
        example: 'У постоянного поставщика могут быть и новые, и б/у вещи. Для б/у товара отметьте обе галочки: в статистике он попадёт в секонд, а в анализ дозаказа размеров не попадёт.'
    },
    'secondhand-product': {
        title: 'Б/у товар (секонд)',
        description: 'Галочка относится только к этой карточке товара. Она не меняет другие товары бренда и признак постоянного поставщика. Отмечено — секонд, не отмечено — новый товар.',
        example: 'Статистика продаж и поступлений использует текущее состояние карточки. Для новых и б/у вещей одного бренда создавайте отдельные карточки. У старых карточек до первого сохранения сохраняется прежняя классификация.'
    }
};
